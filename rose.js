  ...
  _createClass(Rose, [{
    ...
    value: function render() {
      if (!this.fade) {
        ...
        fill(this.color2);
        for (var i = 0; i < this.l; i++) {
          curve(this._points[mod(i - 1, this.l)].x, this._points[mod(i - 1, this.l)].y, this._points[i].x, this._points[i].y, this._points[mod(i + 1, this.l)].x, this._points[mod(i + 1, this.l)].y, this._points[mod(i + 2, this.l)].x, this._points[mod(i + 2, this.l)].y);
        }
      } else {
...
var keyPressed = function keyPressed() {
  if (keyCode == 32) paused = !paused;else if (keyCode == LEFT_ARROW) speed = Math.max(0.1, speed - 0.1);else if (keyCode == RIGHT_ARROW) speed = Math.min(2, speed + 0.1);else if (keyCode == UP_ARROW) lifespan = lifespan + 1;else if (keyCode == DOWN_ARROW) lifespan = Math.max(5, lifespan - 1);else rose.constructor();
};